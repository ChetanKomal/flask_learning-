...
def read_blob(project_name,guid):
    url = f"https://qnai.blob.core.windows.net/{project_name}/{guid}.pdf"
...
def page_range():
    request_url = urllib.request.urlopen(f"https://qnai.blob.core.windows.net/essentials/pageNumbers.txt")
...
def gen_qna():
  ...
  for i in range(0,len(ai_blank_copy)-2):
     ...
     print("<================================================================>")
  upload_blob("ip.txt",f"{guid}.txt")
  table_upload().main(guid)
